...
@interface XTNotificationOberverIdentifer()

@property(nonatomic,strong) NSNotificationCenter *notificationCenterCenter;
@property(nonatomic,strong) dispatch_queue_t dispatchQueue;
@property(nonatomic,copy) XTNotificatioObserverBlock block;
@property(nonatomic,weak) id object;

@end

@implementation XTNotificationOberverIdentifer
...
@interface XTNotificationOberverIdentifersContainer()

@property(nonatomic,strong) NSMutableDictionary *notificationOberverIdentifersDic;

@end

@implementation XTNotificationOberverIdentifersContainer
        ...
        NotificationPerformLocked(^{
            [self modifyContainer:^(NSMutableDictionary *notificationOberverIdentifersDic) {
                ...
                if ([notificationOberverIdentifersDic objectForKey:name]) {
                    XTNotificationOberverIdentifer *identifier = (XTNotificationOberverIdentifer *)[notificationOberverIdentifersDic objectForKey:name];
    ...
    NotificationPerformLocked(^{
        [self modifyContainer:^(NSMutableDictionary *notificationOberverIdentifersDic) {
            ...
            for (NSString *key in notificationOberverIdentifersDic ) {
                XTNotificationOberverIdentifer *identifier = (XTNotificationOberverIdentifer *)[notificationOberverIdentifersDic objectForKey:key];
                [identifier stopObserver];
            }
